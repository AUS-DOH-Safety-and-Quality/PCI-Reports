...
## Demographics
```{r cumulative sum cases, echo=FALSE, fig.width=6, fig.height=4} 
...
ggplot2::ggplot(dplyr::filter(pci_data, !is.na(pci_indication)), ggplot2::aes(x = reorder(stringr::str_wrap(pci_indication, 10), pci_indication, FUN = length, decreasing = T))) +
  ggplot2::geom_bar(fill = "steelblue") +
  ggplot2::geom_text(ggplot2::aes(label = ggplot2::after_stat(count)), stat = "count", vjust = -0.5, size = 3) +
...
```{r pci_indication_list, echo=FALSE}

indication_list <- pci_data |> 
  dplyr::group_by(pci_indication_list) |>
