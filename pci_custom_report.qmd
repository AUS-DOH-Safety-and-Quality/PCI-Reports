...
comorbs <- c("db", "pvd2", "stroke", "dyslipidaemia", "smk", "heart_failure", "htn", "lung_disease","dial")

 gtsummary::tbl_summary(pci_data,
    include = c(db, dbm, pvd2, stroke, dyslipidaemia, smk, heart_failure, htn, lung_disease, dial),
    label = list(
      db ~ "Diabetes",
      dbm ~ "Diabetes Type",
    ...
    statistic = list(
      c(db, dbm, pvd2, stroke, dyslipidaemia, smk, heart_failure, htn, lung_disease, dial) ~ "N: {n} | D: {N} | %: {p}"
    ),
    missing = "no"
  )
