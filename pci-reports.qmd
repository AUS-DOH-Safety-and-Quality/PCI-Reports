...
pci_data |>
  ...
  dplyr::mutate(
    ...
    sex = sex - 1, # Map to 0/1 for gtsummary
...
#   - See the `statistic` argument below for the different statistics calculated
  gtsummary::tbl_summary(
    include = c(age, sex, inds_cat, bmi, severe_obesity, los_cat, db, pvd, ppci, pcabg, shock, oca),
    by = "acs_cat",
    label = list(
      age ~ "Age",
      sex ~ "Female",
    ...
    statistic = list(
      c(age, bmi) ~ "{mean} ({sd})",
      c(sex, inds_cat, db, pvd, severe_obesity, ppci, pcabg, shock, oca, los_cat) ~ "{n} ({p}%)"
    ),
    digits = list(
      c(age, bmi) ~ 1
    ),
...
pci_data |>
  ## 1 NA age record appears in the cohort, remove these values.
  dplyr::filter(!is.na(age) & inds %in% c(1, 2, 3)) |>
  ggplot(aes(x = age_cat, fill = sex_cat, by = 1, y = after_stat(prop))) +
    geom_prop_bar() +
    geom_prop_text() +
    theme_minimal() +
