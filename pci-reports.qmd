...
pci_data <- pci_data |>
  dplyr::mutate(
    ...
    los_cat = factor(los_cat, levels = 1:3, labels = c("Same-Day", "1-5 Days", "5+ Days"))
  )
...
```{r LOS}
pci_data |>
    ggplot(aes(x = hid, fill = forcats::fct_rev(los_cat), y = after_stat(prop))) +
    geom_prop_bar() +
    geom_prop_text() +
    theme_minimal() +
    coord_flip() +
    ...
    # Label axis and Title
    labs(
        title = "Length of Stay by Hospital",
        x = "Hospital",
        y = "Proportion (%)",
        fill = "Length of Stay"
    ) +
