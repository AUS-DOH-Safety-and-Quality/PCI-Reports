...
ecg_to_pci_spc <- pci_data |>
  ...
  dplyr::summarise(num = median(ecgdb, na.rm = T))
...
door_to_pci_spc <- pci_data |>
  ...
  dplyr::group_by(period_start, period_end) |>
  dplyr::summarise(num = median(dbdt, na.rm = T))
...
peri_PCI_stroke_spc <- pci_data |>
  dplyr::group_by(period_start, period_end) |>
  dplyr::summarise(num = sum(ihstr, na.rm = T),
...
ihm_spc <- pci_data |>
  dplyr::group_by(period_start, period_end) |>
  dplyr::summarise(num = sum(NCR5_num, na.rm = T),
                   den = sum(NCR5_den, na.rm = T))
