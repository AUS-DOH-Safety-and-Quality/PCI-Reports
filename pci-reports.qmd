...
## All patients grouped by sex & age group. Negative values are calculted for Female Numerators in order to achieve the desired split plot.
pci_sex_age_neg <- pci_data |>
  ## 1 NA age record appears in the cohort, remove these values.
  dplyr::filter(
    !is.na(age),
    sex_name %in% c("Female", "Male")) |>
  dplyr::group_by(sex_name, age_cat, age_cat_order) |>
  dplyr::summarize(num = dplyr::n(), .groups = 'drop') |>
  dplyr::mutate(
    prop = round(num/sum(num)*100, 1),
    ## Invert num and prop for Female attributes
    num = dplyr::case_when(sex_name == "Female" ~ -num,
                    sex_name != "Female" ~ num),
    prop = dplyr::case_when(sex_name == "Female" ~ -prop,
                    sex_name != "Female" ~ prop)
    ) |>
  dplyr::ungroup()

## All Aboriginal patients grouped by age & sex
pci_aboriginal_age_sex <- pci_data |>
  ## 1 NA age record appears in the cohort, remove these values.
  dplyr::filter(
    !is.na(age),
    sex_name %in% c("Female", "Male"),
    inds_name == "Aboriginal") |>
      dplyr::group_by(sex_name, age_cat, age_cat_order
      ) |>
  dplyr::summarize(num = dplyr::n(), .groups = 'drop') |>
  dplyr::mutate(
...
## Temporarily Import ggplot2 as this is currently not in renv.lock
library(ggplot2)

## Convert to Split Bar Plot
ggplot(pci_sex_age_neg, aes(x = reorder(age_cat, age_cat_order), y = prop, fill = sex_name)) +
  geom_bar(aes(group = sex_name), stat = "identity", position = "stack", width = 0.8) +
  scale_fill_manual(values = c("Female" = "#d62728", "Male" = "#0047AB", "Other" = "grey")) +
  # Insert data labels on the stacked bars, aligned to the sex categories. Reverse negative sign on Female
  geom_text(aes(label = ifelse(prop < -1, -prop, ifelse(prop > 1, prop, ""))), colour = "white", size = 8, position = position_stack(vjust = 0.5)) +
  # Flip coordinates for horizontal bars
  coord_flip() +
  facet_grid(~ sex_name, scales = "free_x", space = "free_x") +
  ...
  theme(
    ...
    # Adjust position of the facet labels
    strip.text = element_text(size = 20, hjust = 0.5, vjust = -1),
    # Styalise and reposition y axis text
    ## axis.text.x = element_text(size = 0, colour = "black"),
    axis.text.x = element_text(size = 0),
    axis.text.y = element_text(size = 20, colour = "black", hjust = 7.5),
...
## Convert to Split Bar Plot
ggplot(pci_aboriginal_age_sex, aes(x = reorder(age_cat, age_cat_order), y = prop, fill = sex_name)) +
  geom_bar(
    aes(group = sex_name),
    stat = "identity",
    position = "stack",
  ...
  # Insert data labels on the stacked bars, aligned to the sex categories. Reverse negative sign on Female
  geom_text(
    aes(label = ifelse(prop < -1, -prop, ifelse(prop > 1, prop, ""))),
    colour = "white",
    size = 8,
  ...
  # Label axis and Title
  labs(
    title = "Aboriginal Breakdown by Age & Sex",
    x = "Age Category",
    y = "Proportion (%)"
  ) +
  theme_minimal() +
  theme(
    ...
    ## axis.text.x = element_text(size = 0, colour = "black"),
    axis.text.x = element_text(size = 0),
    axis.text.y = element_text(size = 20, colour = "black", hjust = 8.7),
    axis.title = element_text(size = 20, colour = "black"),
    plot.title = element_text(size = 24, face = "bold", colour = "black"),
    # Remove legend
    legend.position = "none"
