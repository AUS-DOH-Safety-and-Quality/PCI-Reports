...
knitr::opts_chunk$set(message = FALSE, echo = FALSE, warning = FALSE,  out.width = "100%", screenshot.opts = list(vwidth = 740, vheight = 350, zoom = 1.0), webshot = "webshot")
...
peri_PCI_stroke_spc <- pci_data |>
  dplyr::group_by(period_start, period_end) |>
  dplyr::summarise(indicatorID = "NCR3",
                   num = sum(ihstr, na.rm = T),
                   den = sum(NCR3_den, na.rm = T))
...
#Group data for the SPC function
unplanned_revasc_spc <- pci_data |> 
  dplyr::group_by(period_start, period_end) |>
  dplyr::summarise(indicatorID = "NCR7",
    num = sum(NCR7_num, na.rm = T),
    den = sum(NCR7_den, na.rm = T))

ncr_indicator_data_spc <- rbind(unplanned_revasc_spc, ih_bleeding_spc, antiplatelet_spc, cardiac_rehab_spc,
                                lipid_lowering_spc, mort_30_day_spc, unplanned_readmission_spc, ihm_spc,
                                peri_PCI_stroke_spc, door_to_pci_spc, ecg_to_pci_spc)

ncr_indicator_data_fpl <- rbind(unplanned_revasc_fpl, ih_bleeding_fpl, antiplatelet_fpl, cardiac_rehab_fpl,
                                lipid_lowering_fpl, mort_30_day_fpl, unplanned_readmission_fpl, ihm_fpl,
                                peri_PCI_stroke_fpl)
```
...
qiverse.qiplotly::spc_plotly_create(ecg_to_pci_spc$period_end,
                                    ...
                                    better_is = "Lower",
                                    y_format = "Numeric",
                                    title = "Time from diagnostic ECG to PCI mediated reperfusion. (minutes)",
...
qiverse.qiplotly::spc_plotly_create(door_to_pci_spc$period_end,
                                    ...
                                    better_is = "Lower",
                                    y_format = "Numeric",
                                    title = "Time from door to PCI mediated reperfusion. (minutes)",
                                    spc_period_start = min(pci_data$period_start),
                                    spc_period_end = max(pci_data$period_end),
                                    nhs_icons_enable = T,
                                    x_format = "%b %y") |> 
...
``` {r Indicator_funnel 4}
## Generate the Funnel chart for Indicator 4
tempFile <- paste0(temp, "/NCR4_fpl.html")
qiverse.qiplotly::fpl_plotly_create(
                  numerator = ih_bleeding_fpl$num,
                  denominator = ih_bleeding_fpl$den,
                  group = ih_bleeding_fpl$hname,
                  better_is = "Lower",
                  y_limit_upper = 2,
                  title = "In-Hospital Major Bleeds",
                  funnel_period_start = min(pci_data$period_start),
                  funnel_period_end = max(pci_data$period_end),
                  highlight_outlier = F)|> 
  #Save the plotly plot into a html file - required to output to word
  htmlwidgets::saveWidget(file = tempFile)
#Snapshot the html file and save it as a png
webshot::webshot(url = tempFile, file = "Plots/NCR4_fpl.png")
...
## Generate the SPC chart for Indicator 3
tempFile <- paste0(temp, "/NCR4_fpl.html")
qiverse.qiplotly::spc_plotly_create(ih_bleeding_spc$period_end,
                  numerator = ih_bleeding_spc$num,
                  denominator = ih_bleeding_spc$den,
                  data_type = "p",
                  better_is = "Lower",
                  title = "In-Hospital Major Bleeds",
                  spc_period_start = min(pci_data$period_start),
...
qiverse.qiplotly::fpl_plotly_create(
                  ...
                  better_is = "Lower",
                  y_limit_upper = 10,
                  title = "30 day unplanned cardiac readmission rate after PCI",
                  funnel_period_start = min(pci_data$period_start),
                  funnel_period_end = max(pci_data$period_end),
                  highlight_outlier = F)|> 
  #Save the plotly plot into a html file - required to output to word
...
``` {r Indicator_funnel 7}
## Generate the Funnel chart for Indicator 6
tempFile <- paste0(temp, "/NCR7_fpl.html")
qiverse.qiplotly::fpl_plotly_create(
                  numerator = unplanned_readmission_fpl$num,
                  denominator = unplanned_readmission_fpl$den,
                  group = unplanned_readmission_fpl$hname,
                  better_is = "Lower",
                  y_limit_upper = 10,
                  title = "30 day unplanned revascularisation rate after PCI",
                  funnel_period_start = min(pci_data$period_start),
                  funnel_period_end = max(pci_data$period_end),
                  highlight_outlier = F)|> 
  #Save the plotly plot into a html file - required to output to word
  htmlwidgets::saveWidget(file = tempFile)
#Snapshot the html file and save it as a png
webshot::webshot(url = tempFile, file = "Plots/NCR7_fpl.png")
