...
# Create end_period as last day of month from dop
library(lubridate)
pci_clean <- data %>% mutate(
  PCI_num = ifelse(pci == 1, 1, 0),
  end_period = as.Date(ceiling_date(as.Date(dop), "month") - days(1))
)
cabg_clean <- data %>% mutate(
  CABG_num = ifelse(ihtvcab == 1, 1, 0),
  end_period = as.Date(ceiling_date(as.Date(dop), "month") - days(1))
)
ppci_clean <- data %>% mutate(
  PPCI_num = ifelse(ppci == 1, 1, 0),
  end_period = as.Date(ceiling_date(as.Date(dop), "month") - days(1))
)
...
if (nrow(pci_clean) > 0) {
  controlcharts::spc(
    ...
    keys = pci_clean$end_period,
    spc_settings = list(chart_type = "p"),
    y_axis_settings = list(ylimit_label = "% Patients with PCI", ylimit_label_size = 14),
    outlier_settings = list(improvement_direction = "decrease", shift = TRUE, trend = TRUE, two_in_three = TRUE, astronomical = TRUE),
...
if (nrow(cabg_clean) > 0) {
  ...
  print("No CABG data available.")
}
```

```{r previous-procedures-PPCI}
# SPC Chart for Previous PCI
if (nrow(ppci_clean) > 0) {
  controlcharts::spc(
    data = ppci_clean,
    numerators = ppci_clean$PPCI_num,
    denominators = rep(1, nrow(ppci_clean)),
    keys = ppci_clean$end_period,
    spc_settings = list(chart_type = "p"),
  ...
  print("No Previous PCI data available.")
}
```
